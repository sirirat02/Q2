...
// ดึงช่วงวันที่ต่ำสุดและสูงสุดจากฐานข้อมูล
...
if (($start_date) && ($end_date)) {
    $sql .= checkWhere($sql, "  order_date BETWEEN '$start_date' AND '$end_date'");
} 

// กรองตามช่วงราคา
if($min_price && $max_price) {
    $sql .= checkWhere($sql, "  order_price BETWEEN $min_price AND $max_price");
}


// กรองตามประเภทอุปกรณ์
if ($equipment_type) {
    $sql .= checkWhere($sql, "  equipment_type = '$equipment_type'");
}
            ...
            <div class="sidebar-content">
                ...
                <form id="filterForm" method="GET" action="" onsubmit="return false;">
                    ...
                    <select name="order_quantity" id="filter-quantity-list" class="filter-select">
                        ...
                        <option value="ASC" <?php echo ($order_quantity == "ASC") ? "selected" : ""; ?>>น้อยสุด-มากสุด</option>
                        <option value="DESC" <?php echo ($order_quantity == "DESC") ? "selected" : ""; ?>>มากสุด-น้อยสุด</option>
                    </select>
                    <label for="filter-price">เรียงลำดับราคา:</label>
                    <select name="order_quantity" id="filter-price-list" class="filter-select">
                        <option value="" selected hidden>เลือกการจัดเรียงราคา</option>
                        <option value="">ทั้งหมด</option>
                        <option value="ASC" <?php echo ($order_price == "ASC") ? "selected" : ""; ?>>น้อยสุด-มากสุด</option>
                        <option value="DESC" <?php echo ($order_price == "DESC") ? "selected" : ""; ?>>มากสุด-น้อยสุด</option>
                    </select>
